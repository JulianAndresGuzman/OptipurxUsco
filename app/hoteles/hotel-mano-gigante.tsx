import { useRouter } from 'expo-router';
import React from 'react';
import { Image, ScrollView, StyleSheet, Text, View } from 'react-native';
import MapView, { Marker } from 'react-native-maps';

export default function hotelManoGigante() {
  const router = useRouter();

  const latitude = 2.443014438415437;
  const longitude = -75.48693974558394;

  return (
    <ScrollView style={styles.container}>
      <View style={{ height: 35 }} />
      <Text style={styles.title}>Hotel Bio habitat Entre Montañas</Text>

      <Image
        source={require("../img/hotel-gigante.jpg")}
        style={styles.image}
      />

      <Text style={styles.description}>
        Biohabitat Entre Montañas tiene piscina al aire libre, jardín, terraza y restaurante en Gigante.
        El alojamiento dispone de servicio de habitaciones, servicio de conserjería y servicio de organización de tours.
        En el hotel, las habitaciones tienen armario. 
        Las habitaciones del alojamiento tienen baño privado con ducha y artículos de aseo gratuitos. 
        También incluyen wifi gratis, y algunas habitaciones incluyen vistas al jardín.      
        </Text>

      <View style={styles.mapContainer}>
        <MapView style={styles.map} initialRegion={{ latitude, longitude, latitudeDelta: 0.01, longitudeDelta: 0.01 }}>
          <Marker coordinate={{ latitude, longitude }} />
        </MapView>
      </View>

      <Text style={styles.back} onPress={() => router.back()}>
        ← Volver
      </Text>
    </ScrollView>
  );
}

const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#F9E4C8', padding: 15 },
  title: { fontSize: 26, fontWeight: 'bold', textAlign: 'center', marginBottom: 10 },
  image: { width: '100%', height: 200, borderRadius: 10, marginBottom: 15 },
  description: { fontSize: 16, textAlign: 'justify', color: '#333' },
  mapContainer: { width: '100%', height: 200, borderRadius: 10, overflow: 'hidden', marginTop: 12 },
  map: { flex: 1 },
  back: { textAlign: 'center', marginTop: 16, fontWeight: 'bold', color: '#3B82F6' },
});
